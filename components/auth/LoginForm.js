...
const LoginForm = () => {
    ...
    const handleSubmit = async (e) => {
        ...
        if (!values.nombre.match(nameRegex)) {            
            return; // Evitar que se realice la acción si la validación no pasa
        }
    }
        ...
        <div className="fixed w-screen h-screen inset-0 z-10 flex justify-center items-center bg-blue-400 bg-opacity-25">
            <form onSubmit={handleSubmit} className="bg-gray-100 px-8 pt-6 pb-8 mb-4 rounded-xl shadow-lg w-1/2">
                <div className="mb-4">
                    ...
                    <input className="w-full shadow border border-blue-100 rounded py-2 px-3 text-gray-700 font-mono" type="text" pattern="[A-Za-z]+" title="Solo se permiten letras" placeholder="Tu nombre" value={values.nombre} name="nombre" onChange={handleChange} required />
                </div>
                <div className="mb-4">
